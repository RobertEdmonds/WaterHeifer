...
export default function Donate({ companies, setCompanyName, setTax, setCompanyDescript, setCompanyId, setCompanyEdit, removeCompany }) {
      ...
      {companies.map((company) => {
        return (
          <div key={company.id} className="donateColumn">
            ...
            {users.user.employee && (
              <>
            <Button variant="contained" onClick={() => setEditForm(company)}>Edit</Button>
            <Button variant="contained" onClick={() => deleteCompany(company.id)}>delete</Button>
            </>
            )}
          </div>
        );
